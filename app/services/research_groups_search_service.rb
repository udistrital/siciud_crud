class ResearchGroupsSearchService
    def self.search(curr_research_groups,query)
        curr_research_groups = curr_research_groups.where("name like '%#{query}%'") if query.present?
        ...
        #curr_research_groups = curr_research_groups.joins(members: :researcher).where("researcher.name like 'jorge'")
