...
ActiveRecord::Schema.define(version: 2022_04_22_025344) do
  ...
  create_view "siciud.action_plan_print_views", sql_definition: <<-SQL
      SELECT ap.id,
      ( SELECT json_agg(json_build_object('id', faap.id, 'advanced_total', faap.advanced_total, 'goal', faap.goal, 'order', faap."order", 'indicator_id', faap.indicator_id, 'indicator_description', i.ind_description, 'indicator_product_type_name', si.st_name, 'plan_type_id', faap.plan_type_id, 'plan_type_name', spt.st_name, 'product_type_id', faap.product_type_id, 'product_type_name', spr.st_name)) AS json_agg
             FROM ((((form_a_act_plans faap
      ...
      ( SELECT json_agg(json_build_object('id', fbap.id, 'financing_type_id', fbap.financing_type_id, 'financing_type_name', sft.st_name, 'description', fbap.description, 'goal_state_id', fbap.goal_state_id, 'goal_state_name', sgs.st_name, 'goal_achieved', fbap.goal_achieved, 'order', fbap."order", 'plan_type_id', fbap.plan_type_id, 'plan_type_name', spt.st_name)) AS json_agg
             FROM (((form_b_act_plans fbap
               LEFT JOIN subtypes sft ON ((sft.id = fbap.financing_type_id)))
               LEFT JOIN subtypes sgs ON ((sgs.id = fbap.goal_state_id)))
      ...
      ( SELECT json_agg(json_build_object('id', fdap.id, 'name', fdap.name, 'description', fdap.description, 'goal_state_id', fdap.goal_state_id, 'goal_state_name', sgs.st_name, 'goal_achieved', fdap.goal_achieved, 'order', fdap."order", 'plan_type_id', fdap.plan_type_id, 'plan_type_name', spt.st_name)) AS json_agg
             FROM ((form_d_act_plans fdap
               LEFT JOIN subtypes sgs ON ((sgs.id = fdap.goal_state_id)))
